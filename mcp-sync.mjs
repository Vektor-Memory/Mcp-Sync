#!/usr/bin/env node
/**
 * mcp-sync v0.3.0 — CLI entry point
...
function banner() {
  ...
  console.log(' ' + Co('╚═╝     ╚═╝') + St(' ╚═════╝ ') + Sk('╚═╝     '));
  console.log('');
  console.log('  ' + Si('sync MCP server configs across editors') + '  ' + Gr('· Apache 2.0 · github.com/Vektor-Memory/mcp-sync'));
  console.log('');
}
  ...
  if (fromEditor) {
    ...
    try {
      ...
      if (partial.credentials && Object.keys(partial.credentials).length) {
        ...
        for (const k of Object.keys(partial.credentials)) console.log('     ' + Gr(`mcp-sync vault set ${k} <value>`));
      }
    } catch (err) {
...
async function cmdAdd() {
  ...
  try {
    ...
    console.log('  ' + Gr('  Run ') + Sk('mcp-sync sync') + Gr(' to push to all editors\n'));
  } catch (err) {
    rl.close();
    console.error('\n  ' + R(`✗  ${err.message}`) + '\n'); process.exit(1);
  }
...
async function cmdShare() {
  ...
  try {
    const res = await fetch('https://paste.rs/', {
      method:  'POST',
      headers: { 'Content-Type': 'text/plain', 'User-Agent': 'mcp-sync/0.3.0' },
      body:    content,
    });
    if (!res.ok) throw new Error(`paste.rs HTTP ${res.status}`);
    const url = (await res.text()).trim();
    console.log('  ' + G('✓') + Gr('  Shared → ') + Sk(url));
    console.log('  ' + Gr('  Import with: ') + Sk(`mcp-sync init --from-url ${url}`) + '\n');
  } catch (err) {
...
function cmdProfile() {
  ...
  if (sub === 'save') {
    ...
    if (!name) { console.error('\n  ' + R('✗  Usage: mcp-sync profile save <name>') + '\n'); process.exit(1); }
    ...
    const name = positional(1);
    if (!name) { console.error('\n  ' + R('✗  Usage: mcp-sync profile use <name>') + '\n'); process.exit(1); }
    if (!profiles[name]) {
    ...
    console.log('  ' + Gr('  Run ') + Sk('mcp-sync sync') + Gr(' to push to editors\n'));
    ...
    if (!Object.keys(profiles).length) {
      console.log('  ' + BAR + '  ' + Gr('No profiles saved yet. Use: mcp-sync profile save <name>'));
    } else {
    ...
    const name = positional(1);
    if (!name) { console.error('\n  ' + R('✗  Usage: mcp-sync profile delete <name>') + '\n'); process.exit(1); }
    ...
    console.error('\n  ' + R('✗  Usage: mcp-sync profile <save|use|list|delete> [name]') + '\n');
    process.exit(1);
  }
}
...
async function cmdSearch() {
  ...
  if (!query) {
    console.error('  ' + R('✗  Usage: mcp-sync search <query>') + '\n'); process.exit(1);
  }
    ...
    if (ans) {
      ...
      if (found) {
        try {
          ...
          mcpfile.write(filePath, mcp);
          console.log('\n  ' + G('✓') + Gr('  Added ') + W(`"${found.name}"`) + Gr(' — run ') + Sk('mcp-sync sync') + Gr(' to push\n'));
        } catch (err) {
          console.error('\n  ' + R(`✗  ${err.message}`) + '\n');
        }
      } else {
        console.log('\n  ' + Y('⚠') + Gr(`  "${ans}" not found in results — use `) + Sk('mcp-sync add') + Gr(' to add manually\n'));
      }
    }
  }
}
...
function cmdHelp() {
  ...
  row(W('status'),  Sk('mcp-sync status') +  Gr('               show installed editors & counts'));
  row(W('diff'),    Sk('mcp-sync diff') +    Gr('                 drift check (CI-friendly)'));
  row(W('add'),     Sk('mcp-sync add') +     Gr('                  interactive server wizard'));
  row(W('ping'),    Sk('mcp-sync ping') +    Gr('                 health check all servers'));
  row(W('share'),   Sk('mcp-sync share') +   Gr('                publish config to shareable URL'));
  row(W('profile'), Sk('mcp-sync profile') + Gr(' save|use|list|delete  named server sets'));
  row(W('search'),  Sk('mcp-sync search') +  Gr(' <query>         find MCP servers in registry'));
  row(W('vault'),   Sk('mcp-sync vault') +   Gr('  set|get|delete|list  manage secrets'));
  boxEnd();
  ...
  console.log('  ' + BAR + '  ' + Sk('mcp-sync init --from cursor && mcp-sync sync'));
  blank();
  console.log('  ' + BAR + '  ' + Gr('# Preview what sync would change'));
  console.log('  ' + BAR + '  ' + Sk('mcp-sync sync --dry-run'));
  blank();
  ...
  console.log('  ' + BAR + '  ' + Sk('mcp-sync search filesystem'));
  blank();
  console.log('  ' + BAR + '  ' + Gr('# Health-check all configured servers'));
  console.log('  ' + BAR + '  ' + Sk('mcp-sync ping'));
  blank();
  console.log('  ' + BAR + '  ' + Gr('# Save a named profile, switch later'));
  console.log('  ' + BAR + '  ' + Sk('mcp-sync profile save work'));
  console.log('  ' + BAR + '  ' + Sk('mcp-sync profile use personal'));
  blank();
  console.log('  ' + BAR + '  ' + Gr('# Share config with a teammate'));
  console.log('  ' + BAR + '  ' + Sk('mcp-sync share'));
  blank();
  boxEnd();
}
