\# mcp-sync Portable Format Specification
...
It defines MCP servers once and lets `mcp-sync` write the correct client-specific
...
| `meta`    | object | no       | Sync metadata — written by mcp-sync, not by hand |
...
| `note`      | string   | no       | Human comment — ignored by mcp-sync                   |
...
mcp-sync vault set github-token ghp\_xxxx   # store

mcp-sync vault get github-token            # retrieve

mcp-sync vault list                         # list all keys

mcp-sync vault delete github-token         # remove
...
\## Meta Block (written by mcp-sync)
...
Do not edit this block manually — mcp-sync owns it.
...
mcp-sync export --from claude-desktop



\# Write .mcp.json → single client

mcp-sync import --to cursor
...
mcp-sync sync --clients claude-desktop,cursor,vscode
