 ...
 *
 * Reads and writes Gemini CLI MCP config.
 *
 * Config file:
 *   Win   : %APPDATA%\.gemini\settings.json
...
export function sync(mcpJson) {
  ...
  for (const [name, cfg] of Object.entries(incoming))
    for (const [k, v] of Object.entries(cfg.env ?? {}))
      if (looksLikeSecret(v)) console.warn(`  gemini: ${name}.env.${k} looks like a plaintext secret — use vault:key-name`);
