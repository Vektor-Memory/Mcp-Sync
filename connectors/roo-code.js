/**
 * connectors/roo-code.js
 *
 * Reads and writes Roo Code MCP config.
 * Extension ID: rooveterinaryinc.roo-cline
 *
 * Config file locations (platform-specific globalStorage path):
 *   Win   : %APPDATA%\Code\User\globalStorage\rooveterinaryinc.roo-cline\settings\cline_mcp_settings.json
 *   macOS : ~/Library/Application Support/Code/User/globalStorage/rooveterinaryinc.roo-cline/settings/cline_mcp_settings.json
 *   Linux : ~/.config/Code/User/globalStorage/rooveterinaryinc.roo-cline/settings/cline_mcp_settings.json
 *
 * Root key: "mcpServers" — same shape as Claude Desktop and Cline.
 */
  ...
  for (const [name, cfg] of Object.entries(incoming)) {
    for (const [k, v] of Object.entries(cfg.env ?? {})) {
      if (looksLikeSecret(v))
        console.warn(`  roo-code: ${name}.env.${k} looks like a plaintext secret — use vault:key-name`);
    }
  }
